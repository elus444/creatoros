...
function ContentLibrary() {
  ...
  const emptyCopy = useMemo(() => {
    ...
    return {
      ...
      body: "Select a trend and generate a content package to open the workspace.",
      href: "/trends",
      label: "Go to trends",
    };
  }, [projects.length]);
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-xl text-sm leading-6 text-on-surface-variant md:text-base">
            Your content library — open any package in the workspace to edit, review,
            approve, and export.
          </p>
        </div>
        <Button asChild>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-on-surface-variant">
                      {item.project_name || "Project"} · {timeAgo(item.created_at)}
                    </p>
                  </div>
                  <span
                    className={cn(
