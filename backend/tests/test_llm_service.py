...
@pytest.mark.asyncio
async def test_unparseable_provider_response_raises_retryable_response_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _BadJsonResponse(_FakeResponse):
        def __init__(self) -> None:
            self.status_code = 200
            self.text = "<html>temporary proxy error</html>"
    ...
    with pytest.raises(LLMResponseError, match="unparseable JSON response"):
        await LLMService().generate_structured(prompt="Go.", response_model=_DummySchema)
