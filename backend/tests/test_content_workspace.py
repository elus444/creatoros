...
from app.schemas.agent_outputs import ContentOutput, ResearchOutput, StrategyOutput, SuggestionOutput
...
VALID_CONTENT = {
    ...
    "caption": "The easiest pasta you'll make all week.",
...
def _create_generated_content(
    ...
    response = client.post(
        "/api/v1/content/generate", json={"trend_id": trend_id}, headers=headers
    )
...
def test_status_flow_generated_review_approve_export(
    ...
    assert "Revised" not in body["body"] or "Open on a single pan" in body["body"]
    ...
    _install_fake_llm(
        monkeypatch,
        {
            ContentOutput: [
                _llm_result(
                    {
                        "script": "Brand new regenerated script.",
                        "titles": ["Regen Title"],
