...
def test_growth_velocity_beats_stale_popularity() -> None:
    now = datetime.now(tz=UTC)
    rising = scoring.score_item(
        "youtube",
        {"views": 40_000, "likes": 3_500, "comments": 400},
        now - timedelta(hours=12),
    )
    stale_hit = scoring.score_item(
        "youtube",
        {"views": 900_000, "likes": 4_000, "comments": 200},
        now - timedelta(days=12),
    )
    assert rising > stale_hit
    assert rising != stale_hit
